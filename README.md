...
환자 개인의 복약 기록과 부작용 증상 이력(Personal Graph)은 Mysql, 일반 의약품 병용 금기 데이터(Medical Graph)는 Neo4j, 최종적으로 Graph + Vector 하이브리드 검색으로 근거를 제시하는 개인화 상담 챗봇을 Django 웹 애플리케이션으로 구축했습니다.
